...
```
Electron App  →  IPC (main process)  →  flowrytr-api (Vercel)  →  Provider (transcription)
...
|------|-----|-------|---------------|-----------|
| flowrytr Cloud (default) | Signed-in users | 2000 words/day free, unlimited Pro | Vercel API → provider | Vercel API → provider |
...
## Step 1: Create `flowrytr-api` Repo

Location: `~/Projects/flowrytr/flowrytr-api/`
...
- **Limit logic**: Post-transcription check. The audio is already transcribed (API cost incurred) — don't waste it. `limitReached: true` tells the client the _next_ request will be blocked. Pre-flight check via `GET /api/usage` handles UI state.
...
- `cloudTranscriptionMode`: `"flowrytr"` | `"byok"` (default: `"flowrytr"`)
...
- Add `processWithflowrytrCloud(audioBlob, metadata)` method:
  ...
  - Returns same `{ success, text, source: "flowrytr", timings }` shape as other methods
...
- Modify `processAudio()` routing (~line 219):
  ...
  else if (cloudTranscriptionMode === "flowrytr" && isSignedIn) → processWithflowrytrCloud (NEW)
  else → processWithOpenAIAPI (existing BYOK flow, unchanged)
  ```

**`src/services/ReasoningService.ts`** — Add `"flowrytr"` provider

- Add `processWithflowrytr()` method alongside existing `processWithOpenAI()`, `processWithAnthropic()`, etc.
- Calls `window.electronAPI.cloudReason(text, { model, agentName, customDictionary })`
- When `cloudTranscriptionMode === "flowrytr"` && signed in, `reasoningProvider` is `"flowrytr"`
...
- When signed in: default to flowrytr Cloud — show language picker + reasoning model selector (curated list from `modelRegistryData.json`, smart default pre-selected)
...
- **Transcription section**: Mode toggle — "flowrytr Cloud" (simple) vs "Use your own API key" (shows existing provider/model/key pickers)
...
- Add `"flowrytrCloudModels"` section with curated tiers:
...
- Add `FLOWRYTR_API_URL` constant (from `VITE_API_URL` env var, used by main process IPC handlers)
...
│                                                      │
│  flowrytr handles transcription and AI             │  ← text-neutral-600
...
```
┌─────────────────────────────────────────────────┐
│  ☁  flowrytr Cloud                    ● ──┐   │  ← selected: border-indigo-500/30, bg-indigo-50/30
...
- **Cloud selected**: Everything below is hidden. Clean empty state — maybe a single line: "Transcription is handled by flowrytr's servers. Change your language in General settings."
...
When `cloudTranscriptionMode === "flowrytr"`, replace `ReasoningModelSelector` with a simplified cloud model picker:
...
│  └───────────────────────────────────────────┘   │
│                                                  │
│  No API key needed. Powered by flowrytr.       │  ← text-xs text-neutral-400
│                                                  │
└─────────────────────────────────────────────────┘
```
...
- Model data from `modelRegistryData.json` `flowrytrCloudModels` section (ships with app, works offline)
...
4. ✅ `git init` new repo at `~/Projects/flowrytr/flowrytr-api/`
...
15. ✅ Add `processWithflowrytrCloud()` to `audioManager.js`
16. ✅ Wire transcription routing in `processAudio()`
17. ✅ Add `"flowrytr"` provider to `ReasoningService.ts`
