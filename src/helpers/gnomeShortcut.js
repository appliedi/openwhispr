...
const DBUS_SERVICE_NAME = "com.flowrytr.App";
const DBUS_OBJECT_PATH = "/com/flowrytr/App";
const DBUS_INTERFACE = "com.flowrytr.App";

const KEYBINDING_PATH =
  "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/flowrytr/";
...
class GnomeShortcutManager {
  ...
  _createInterfaceClass(dbusModule, callback) {
    class FlowrytrInterface extends dbusModule.interface.Interface {
      constructor() {
    ...
    FlowrytrInterface.configureMembers({
      methods: {
        Toggle: { inSignature: "", outSignature: "" },
      },
    });

    return FlowrytrInterface;
  }
  ...
  async registerKeybinding(shortcut = "<Alt>r") {
    ...
    try {
      ...
      execFileSync(
        "gsettings",
        ["set", `${KEYBINDING_SCHEMA}:${KEYBINDING_PATH}`, "name", "flowrytr Toggle"],
        { stdio: "pipe" }
      );
      execFileSync(
        "gsettings",
