...
class ReasoningService extends BaseReasoningService {
  ...
  async processText(
    ...
    if (!trimmedModel && provider !== "flowrytr") {
    ...
    try {
      ...
      switch (provider) {
        ...
        case "groq":
          ...
          break;
        case "flowrytr":
          result = await this.processWithFlowrytr(text, model, agentName, config);
          break;
        case "custom":
          result = await this.processWithOpenAI(text, trimmedModel, agentName, config);
          break;
  ...
  private async processWithFlowrytr(
    text: string,
    model: string,
    agentName: string | null = null,
    config: ReasoningConfig = {}
  ): Promise<string> {
    logger.logReasoning("FLOWRYTR_START", { model, agentName });
    ...
    try {
      ...
      const result = await withSessionRefresh(async () => {
        ...
        if (!res.success) {
          const err: any = new Error(res.error || "flowrytr cloud reasoning failed");
          err.code = res.code;
          throw err;
        }
      ...
      logger.logReasoning("FLOWRYTR_SUCCESS", {
        model: result.model,
      ...
      logger.logReasoning("FLOWRYTR_ERROR", {
        model,
        error: (error as Error).message,
      });
      throw error;
